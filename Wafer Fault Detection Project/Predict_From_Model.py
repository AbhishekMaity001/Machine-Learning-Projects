...
class prediction:
    ...
    def prediction_From_Model(self):
        ...
        try :
            ...
            clusters = kmeans.predict(data.drop(['Wafer'],axis=1)) # dropping the first column for cluster prediction
            data['clusters'] = clusters
            NoOfClusters = data['clusters'].unique()
            for i in NoOfClusters:
                ...
                results = list(model.predict(cluster_data))
                results = pd.DataFrame(list(zip(wafer_names,results)),columns=['Wafer','Prediction'])
                path = "Prediction_Output_File/Predictions.csv"
                results.to_csv(path,header=True,mode='a+')
